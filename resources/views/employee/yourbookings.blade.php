...
<head>
	...
	<link rel="stylesheet" type="text/css" href="{{ asset('CSS/ybooking.css') }}">
</head>
</head>
<body>
	<div class="logo">
    ...
    <div class="bookings-container">
        @foreach($bookings as $booking)
            @if ($booking->status == 0)
                ...
                @endphp
                    <p>{{ $roomName }}</p>
                    <span class="first">{{ $booking->booking_date }} |</span>
                    <span>{{ $booking->time_start }} -</span>
                    <span>{{ $booking->time_end }} |</span>
                    <span>{{ $booking->name }}</span>
                    <input type="submit" class="cancel" name="Pesan" value="Cancel" data-booking-id="{{ $booking->id }}" id="sendButton">
                    ...
                    <script src="{{ asset("JS/popup-ybooking.js") }}"></script>
                </div>
