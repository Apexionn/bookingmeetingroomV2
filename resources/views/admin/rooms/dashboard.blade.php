...
<body>
    <div class="logo">
        <a href="/rooms"><img src="{{ asset('IMG/logo.png') }}"></a>
    </div>
    <ul class="li-nav">
        <li>
            <div class="popover">
...
<div class="container-search">
    <form class="search-form" role="search" action="/rooms">
        {{ csrf_field() }}
    ...
    $(document).ready(function(){
        $('#search').keyup(function(){
            ...
            if(query != '')
            {
                ...
                $.ajax({
                    url:"{{ route('autocomplete-rooms.fetch') }}",
                    method:"POST",
...
<div class="table-container">
                ...
                @foreach ($rooms as $room)
                    <tr class="table-row {{ $no % 2 == 0 ? 'even-row' : 'odd-row' }}">
                        ...
                        <td>
                            @php
                            @endphp
                            <img class="foto" src="{{ asset('images/'.$room->image) }}" alt="" style="width: 80px">
                        </td>
                        <td scope="row">
                            <div class="tes">
                                <a href="/select-room/{{ $room->id }}" class="edit">Edit</a>
                                <a href="/delete-room/{{ $room->id }}" class="delete">Delete</a>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <br>
    <button class="btn" onclick="window.location = 'create-room'">
        <a href="create-room" class="text-decoration-none text-light">ADD Data</a>
    </button>
